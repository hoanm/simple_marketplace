...
// gas price
const gasPrice = GasPrice.fromString(`0.025${
    chainConfig.denom
}`);
...
async function instantiate(contract_code_id, instantiateMsg) {
    ...
    const instantiateResponse = await deployerClient.instantiate(deployerAccount.address, Number(contract_code_id), instantiateMsg, "instantiation contract", "auto",);
...
async function execute(userClient, userAccount, contract, executeMsg, native_amount = 0, native_denom = chainConfig.denom) {
    ...
    if (native_amount != 0) {
        executeResponse = await userClient.execute(userAccount.address, contract, executeMsg, "auto", memo, [coin(native_amount, native_denom)],);
    } else {
...
async function main() {
    ...
    deployerWallet = await DirectSecp256k1HdWallet.fromMnemonic(chainConfig.deployer_mnemonic, {prefix: chainConfig.prefix});
    ...
    // ****************
    // store contract
    let storeCodeResponseMarketplace = await store_contract("marketplace");

    let storeCodeResponseCw721 = await store_contract("cw721_base");

    // prepare instantiate message for cw20 token
    const marketplaceInstantiateMsg = {
        owner: deployerAccount.address,
        collection_code_id: storeCodeResponseCw721.codeId
    }

    let marketplaceInstantiateResponse = await instantiate(storeCodeResponseMarketplace.codeId, marketplaceInstantiateMsg);

    console.log(marketplaceInstantiateResponse);
}
